(ns commitledger.edge.base58
  "base58btc (Bitcoin alphabet) decode+encode for edge did:key parsing --
  a direct, faithful port of `cloud_itonami.edge.base58`
  (`orgs/gftdcojp/cloud-itonami`, AGPL-3.0-or-later, same license as this
  repo -- see `docs/adr/0002-http-edge-live-registry-verification.md`
  for the license-compatibility check), local-mirrored rather than
  cross-repo required -- same reasoning as this repo's own
  `commitledger.registry/equity-blocklist` mirror of `cloud_itonami.
  funding`'s blocklist (see that ns's docstring): this workspace's
  established convention for actor repos is \"shared data contract /
  wire format, no shared code across the repo boundary\". The algorithm
  itself is UNCHANGED from upstream -- do not reinvent the wire format.

  CLJS-only (like upstream: `js/` interop throughout, no JVM branch --
  this runs in the Cloudflare Workers isolate, not the JVM). Decodes/
  encodes via the standard multiply-by-58-with-carry byte algorithm
  rather than an arbitrary-precision-integer approach, since
  ClojureScript has no bignum in `cljs.core` and the byte algorithm needs
  none -- every intermediate value stays under 256*58."
  )
